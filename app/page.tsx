...
const equity = dashboardData.equity_curve;
const drawdown = dashboardData.drawdown_curve;
const historical = dashboardData.historical;
const forward = dashboardData.forward;
  ...
  return (
    <main className="app-shell">
      ...
      <section className="workspace">
        <header className="topbar"><button className="menu-button" onClick={() => setSidebarOpen(true)} aria-label="開啟選單"><Menu /></button><div><h1>策略控制台</h1><p>V37.3 永續合約研究系統</p></div><div className="top-actions"><span className="online"><i />真實快照</span><span className="utc">{dashboardData.cron.last_success_utc ?? '尚未有成功紀錄'}</span><div className="avatar">L</div></div></header>
        <div className="content">
          ...
          <div className="dashboard-grid">
            ...
            <section className="panel status-panel"><div className="panel-title"><h3>策略運行狀態</h3><span className="healthy"><i />安全</span></div><StatusRow label="執行模式" value={dashboardData.mode} good/><StatusRow label="升級狀態" value={`promoted = ${dashboardData.promoted}`}/><StatusRow label="Forward readiness" value={forward.ready_for_review ? 'REVIEW REQUIRED' : 'NOT READY'}/><StatusRow label="最後排程" value={dashboardData.cron.state === 'healthy' ? '成功' : '需要檢查'} good={dashboardData.cron.state === 'healthy'}/><StatusRow label="真實訂單送出" value={String(dashboardData.orders_submitted)} good/><div className="progress-block"><div><span>Forward 驗證</span><b>{Math.min(100, Math.round(forward.days / forward.minimum_days * 100))}%</b></div><div className="progress-track"><i style={{width:`${Math.min(100, forward.days / forward.minimum_days * 100)}%`}}/></div><p>需要 {forward.minimum_days} 日及最少 {forward.minimum_adjustments} 次調整後再作人工審核</p></div></section>
            <section className="panel drawdown-panel"><div className="panel-title"><div><span>歷史 OOS 模擬</span><h3>回撤曲線</h3></div><b className="negative">最低 -{pct(historical.oos_max_drawdown)}</b></div><div className="drawdown-chart"><ResponsiveContainer width="100%" height="100%"><AreaChart data={drawdown} margin={{top:8,right:8,left:-18,bottom:0}}><defs><linearGradient id="ddFill" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#ff667d" stopOpacity={0.05}/><stop offset="100%" stopColor="#ff667d" stopOpacity={0.24}/></linearGradient></defs><CartesianGrid stroke="#2b2429" vertical={false} strokeDasharray="3 3"/><XAxis dataKey="date" hide/><YAxis domain={[-10,0]} tick={{fill:'#71877e',fontSize:11}} axisLine={false} tickLine={false}/><Tooltip contentStyle={{background:'#0b1b16',border:'1px solid #4b2931',borderRadius:10}}/><Area type="monotone" dataKey="value" stroke="#ff667d" strokeWidth={2} fill="url(#ddFill)"/></AreaChart></ResponsiveContainer></div></section>
          ...
          <section className="bottom-grid"><article className="mini-panel"><Server/><div><span>排程服務</span><b>最近一次成功</b><small>{dashboardData.cron.last_success_utc ?? '尚未有紀錄'}</small></div><i className="status-dot"/></article><article className="mini-panel"><Database/><div><span>合約資料宇宙</span><b>{historical.current_weex_specs.toLocaleString()} 個可用規格</b><small>公開市場資料</small></div><i className="status-dot"/></article><article className="mini-panel"><Radar/><div><span>目標覆蓋率</span><b>{pct(historical.oos_target_coverage)}</b><small>OOS 平均值</small></div><i className="status-dot"/></article><article className="mini-panel"><CircleGauge/><div><span>資金配置</span><b>{historical.initial_capital.toFixed(0)} USDT 基準</b><small>50% gross target</small></div><i className="status-dot amber"/></article></section>
        </div>
      </section>
